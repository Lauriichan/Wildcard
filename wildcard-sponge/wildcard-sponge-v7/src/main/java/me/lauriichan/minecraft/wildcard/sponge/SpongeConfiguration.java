...
public final class SpongeConfiguration {
    ...
    public void reload() {
        ...
        try {
            load(HoconConfigurationLoader.builder().setSource(source::openReader).build().load());
        } catch (IOException e) {
    ...
    private void load(CommentedConfigurationNode config) {
        serverName.replace(config.getNode("serverName").getString(serverName.get())); // Just use set name as default value
    }
