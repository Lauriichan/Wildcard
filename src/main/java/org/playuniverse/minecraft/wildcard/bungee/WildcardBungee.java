...
public final class WildcardBungee extends Plugin implements IWildcardPlugin {
    ...
    public void onSetup() {
        ...
        setup = true;
        core.preSetup();
        service.replace(new BungeeService(adapter.getComponentParser(), Singleton.get(PluginSettings.class)));
        ProxyServer.getInstance().getPluginManager().registerListener(this, new PlayerListener(getAdapter(), core.getDatabase()));
        core.postSetup();
    }
