...
import org.playuniverse.minecraft.wildcard.core.command.api.nodes.ExecutionNode;
import org.playuniverse.minecraft.wildcard.core.command.api.nodes.LiteralNode;
...
public final class WildcardCommand implements IBasicCommand {
    ...
    private void onHistory(final CommandContext<BaseInfo> context) {
        ...
        if (page - 1 >= 0) {
            final TextComponent[] current = info.translate("command.history.list.page.previous", "page", page);
            info.apply(current, new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/wildcard history " + targetCmd + (page - 1)),
                new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text(info.translate("command.action.page.previous"))));
            output = Arrays.merge(TextComponent[]::new, output, current);
        }
        output = Arrays.merge(TextComponent[]::new, output, info.translate("command.history.list.page.info", "page", page + 1));
        if (page + 1 < pages) {
            final TextComponent[] current = info.translate("command.history.list.page.next", "page", page + 2);
            info.apply(current, new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/wildcard history " + targetCmd + (page + 1)),
                new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text(info.translate("command.action.page.next"))));
            output = Arrays.merge(TextComponent[]::new, output, current);
        }
        info.send(output);
    ...
    private void onCreate(final CommandContext<BaseInfo> context) {
        ...
        if (!info.isPlayer() || reader.hasNext()) {
            ...
            final TextComponent[] components = info.translate("command.create.other.notify");
            info.apply(components, new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/wildcard get"),
                new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text(info.translate("command.action.token.get"))));
    ...
    private TextComponent[] getTokenMessage(final BaseInfo info, final String message, final String user, final Token token) {
        final TextComponent[] components = info.translate(message, "user", user, "token", token.getToken(), "uses", token.getUses(),
            "expires", token.getExpiresAsString());
        info.apply(components, new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, token.getToken()),
            new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text(info.translate("command.action.token.copy"))));
        return components;
    }
