...
public class WindowsShortcut {
    ...
    public static boolean isPotentialValidLink(final File file) throws IOException {
        ...
        try {
            isPotentiallyValid = file.isFile() && file.getName().toLowerCase().endsWith(".lnk") && fis.available() >= minimum_length
    ...
    public static File lookUp(final File file) {
        if (file.exists()) {
            return file;
        }
        final File link = new File(file.getPath() + ".lnk");
        if (link.exists()) {
            try {
                if (!isPotentialValidLink(link)) {
                    return file;
                }
                return new File(new WindowsShortcut(link).getRealFilename());
    ...
    private void parseLink(final byte[] link) throws ParseException {
        try {
            ...
            if (isLocal) {
                ...
                final int shareName_offset = link[networkVolumeTable_offset + shareName_offset_offset] + networkVolumeTable_offset;
